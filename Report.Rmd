...
In order to address this question, initially download the given file "gene_expression.tsv” contains RNA-seq count data for two samples of interest.
Therefore use the command download.file ("URL",destfile="name"). In this command, destfile="name" used to give the destination and name for the downloaded file.
...
then looked the first six genes using head(x) and str(x)
...
rowmeans(x[,c(2:3)]) command will give a output where mean of the rows will be calculated by considering columns 2 and 3. Comma used as soon as square brackets open depicts the fact that this function consider all the rows.
Therfore full code can be write as x$meanofsamples<-rowMeans(x[,c(2:3)]) and to chek the means for first six genes run x[1:6,]
Spefic columns on the table to run the command will be selected by using the syntax($).
...
by sorting the data it is possible to list the genes with highest mean.
sort() is used to arange the  data set in ascending or discending order.
In order to select the 10 genes run the head command head(x_sorted,10).
...
Histogram were drawn using hist() function and for easy identification break it to 30 frequencies.histogram were drawn in mean of th samples in the x axis and frequancy was taken to y axis.To save the image in png format export the drawn graph and save the option under png format.
...
>>>>>>> af7bd9eb020db44845d7d8b854447e2fd30a7b88


## Question 6 -(Import this csv file into an R object. What are the column names?)


Initially run args() to find the arguments needed to read a csv file.Then,In order to make an object, the data set needed to be saved under a name in environment.
...
The link with fa.gz extension is selected. fa.gz extension was given to destile name.
